// https://github.com/expo/expo/blob/master/home/components/Camera.android.tsx
...
export default function AndroidCamera({ style, cameraRef, ...rest }) {
  const [dimensions, onLayout] = useComponentDimensions()
  const [suggestedAspectRatio, suggestedDimensions, ref] = useAutoSize(
    dimensions,
    cameraRef
  )
...
function useAutoSize(dimensions, cameraRef) {
  const [supportedAspectRatios, ref] = useSupportedAspectRatios(cameraRef)
...
function useSupportedAspectRatios(cameraRef) {
